...
class SessionCategoryTable extends BaseTable {
    ...
    public function getCategoryParent($id){
        $category = ModelsCourseCategory::find($id);
    ...
    public function getChildren($id){
        $categories = ModelsCourseCategory::where('parent_id',$id)->orderBy('sort_order')->where('enabled',1)->get();
    ...
    public function getCategoriesForParent($parentId,$level=0,$exId=null){
        ...
        if(empty($parentId)){
            $categories = ModelsCourseCategory::whereNull('parent_id')->orderBy('sort_order')->get();
        }
        else{
            $categories = ModelsCourseCategory::where('parent_id',$parentId)->orderBy('sort_order')->get();
        }
