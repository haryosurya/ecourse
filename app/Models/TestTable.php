...
class TestTable extends BaseTable{
    ...
    public function getStudentTestRecords($studentId,$testId){
        ...
        $select1->join($this->getPrefix().'course_tests','student_courses.course_id=course_tests.course_id',array('opening_date','closing_date'))
            ->join($this->getPrefix().'tests','course_tests.test_id='.$this->getPrefix().'tests.id',['test_id'=>'id','name','enabled','minutes','allow_multiple','passmark','private'])
    ...
    public function getStudentRecords($studentId){
        ...
        $select1 = new Select('student_courses');
        $select1->join($this->getPrefix().'course_tests',$this->getPrefix().'student_courses.course_id='.$this->getPrefix().'course_tests.course_id',array())
                ->join($this->getPrefix().'tests',$this->getPrefix().'course_tests.test_id='.$this->getPrefix().'tests.id',['test_id'=>'id','name','enabled','minutes','allow_multiple','passmark','private','show_result'])
                ->where([$this->getPrefix().'student_courses.student_id'=>$studentId])
                ->where([$this->getPrefix().'tests.enabled'=>'1'])
                ->where($this->getPrefix()."course_tests.opening_date < $today OR ".$this->getPrefix()."course_tests.opening_date=0")
                ->where($this->getPrefix()."course_tests.closing_date > $today OR ".$this->getPrefix()."course_tests.closing_date=0")
                ->columns([])
                ->group($this->getPrefix().'course_tests.test_id')
            ->order($this->getPrefix().'tests.created_at desc');
    ...
    public function getLastSortOrder($testId){
        $row = ModelsTest::where('id',$testId)->orderBy('sort_order','desc')->first();
