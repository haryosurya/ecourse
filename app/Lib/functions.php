...
function setting($key,$default=null){
    ...
    $setting = Setting::where('key',trim(strtolower($key)))->first();
...
function getApiStudent($request){
    ...
    if(!empty($authToken)){
        $student = \App\Models\Student::where('api_token',$authToken)->first();
    }
    else{
        $student = false;
    }
...
function currencies(){
    // $currencies = \App\Models\Currency::orderBy('currency_id','desc')->get();
    // return $currencies;
}
...
function adminName($adminID){
    if (\App\Models\Admin::find($adminID)){
        return \App\Models\Admin::find($adminID)->user->name;
    }
    else{
        return 'N/A';
    }
...
function headerMenu(){
    ...
    $items= HeaderMenu::where('parent_id',0)->orderBy('sort_order')->get();
    $menu = [];
    $counter = 0;
    foreach($items as $item){
        ...
        //check for children
        $children = HeaderMenu::where('parent_id',$item->id)->orderBy('sort_order')->get();
        if($children->count()>0){
...
function footerMenu(){
    ...
    $items= FooterMenu::where('parent_id',0)->orderBy('sort_order')->get();
    $menu = [];
    $counter = 0;
    foreach($items as $item){
        $menu[$counter] = [
        ...
        //check for children
        $children = FooterMenu::where('parent_id',$item->id)->orderBy('sort_order')->get();
        if($children->count()>0){
            foreach($children as $child){
                $menu[$counter]['children'][] = [
                    'label'=> $child->label,
...
function paymentOption($directory,$option){
    $gateway = PaymentMethod::where('directory',$directory)->first();
    if(!$gateway){
        return false;
    }
