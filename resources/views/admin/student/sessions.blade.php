             ...
             <div class="col-xl-3 col-lg-4 col-md-6">
                <div class="card">
                    <div class="card-body text-center">
                        ...
                        <span><a href="{{ route('admin.student.editsession',['id'=>$row->id])  }}">{{ $row->name }}</a></span>
                        <div class="text-muted"> <a href="#" onclick="openModal('{{ __lang('students-for') }} {{ $row->name}}','{{ route('admin.student.sessionenrollees',['id'=>$row->id])  }}')">{{ $studentSessionTable->getTotalForSession($row->id) }} {{ __lang('students') }}</a></div>
                        @if(!empty($row->picture))
                            <img  src="{{ resizeImage($row->picture,671,480,basePath()) }}" alt="">
                        @else
                            <img   src="{{ asset('img/course.png') }}" alt="">
                        @endif
                        @if(\App\Models\Admin::find($row->admin_id))
                        <div class="article-user">
                            <img alt="image" class="card-profile-img"  src="{{ profilePictureUrl(\App\Models\Admin::find($row->admin_id)->user->picture) }}">
